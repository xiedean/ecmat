<?php
class Admin_UsersyangController extends Main_AdminController
{
	...
	public function indexAction()
	{
		...
		$groups = new Groups();
		$this->view->groupItems = $groups->getGroupsBySite(2);
		$this->dataList();
	...
	public function editAction()
	{
		$this->view->navArray = array(
		                              '用戶列表'=>$this->view->url(array('controller'=>'usersyang','module'=>'admin'),null,true),
		                              'title' => '添加新用戶'
		                             );
		$this->view->itemName = '用戶';
		// filter input
	...
	public function deleteAction()
	{
		...
		if($this->_request->isPost()){
			...
			if( $input->getEscaped('submit-y')){
				...
				if($result){
					$this->_redirect('/admin/usersyang');
				}
				else{
	...
	public function changegroupAction()
	{
	    ...
	    if(is_array($ids)) {
	        foreach ($ids as $id) {
	            if($where) 
	                $where .= $table->getAdapter()->quoteInto(' AND user_id = ?', $id);
	            else
